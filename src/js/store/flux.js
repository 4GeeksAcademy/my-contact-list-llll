const getState = ({ getStore, setStore }) => {
	return {
		...
		actions: {
			...
			getAllContacts: () => {
				fetch("https://playground.4geeks.com/contact/agendas/diegog/contacts")
					.then(response => {
			...
			addNewContact: (data) => {
				return fetch("https://playground.4geeks.com/contact/agendas/diegog/contacts", {
					method: "POST",
			...
			deleteContact: (id) => {
				...
				return fetch(`https://playground.4geeks.com/contact/agendas/diegog/contacts/${id}`, {
					method: "DELETE"
					, headers: { "Content-Type": "application/json" }
				})
					.then(response => {
			...
			editContact: async (id, updatedData) => {
				try {
					const response = await fetch(`https://playground.4geeks.com/contact/agendas/diegog/contacts/${id}`, {
						method: "PUT",
						headers: {
